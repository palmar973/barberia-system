...
class DashboardView(QWidget):
    ...
    def crear_tarjeta_kpi(self, titulo, valor, color):
        """Crea una tarjeta KPI (Corregido para evitar glitch visual)."""
        frame = QFrame()
        # 1. Bautizamos el objeto para que el CSS sea específico
        frame.setObjectName("kpi_frame")
        
        # 2. Usamos el selector por ID (#kpi_frame) en lugar del genérico (QFrame)
        ...
        layout = QVBoxLayout()
        # Ajustamos márgenes para que el texto no quede pegado al borde
        layout.setContentsMargins(10, 10, 10, 10) 
        frame.setLayout(layout)
        
        lbl_titulo = QLabel(titulo)
        lbl_titulo.setFont(QFont("Arial", 12, QFont.Bold))
        # 3. Importante: Fondo transparente explícito
        lbl_titulo.setStyleSheet("background-color: transparent; color: white;")
        lbl_titulo.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        lbl_valor = QLabel(valor)
        lbl_valor.setFont(QFont("Arial", 28, QFont.Bold))
        # 3. Importante: Fondo transparente explícito
        lbl_valor.setStyleSheet("background-color: transparent; color: white;")
